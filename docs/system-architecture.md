...
**Billing → Funds Release cross-BC integration:**

When a facture with a `project_id` transitions to `paid`, the `UpdateBillingDocumentStatusUseCase` calls `FundsReleasePort.create_funds_release()` which creates a `released_funds` invoice in the project's expenses with `is_auto_generated=true` and `source_billing_document_id` set to the facture UUID. A `UNIQUE` constraint on `source_billing_document_id` prevents duplicates (adapter catches `IntegrityError`). When PAID→CANCELLED (refund), `delete_funds_release()` removes the auto-generated invoice. Auto-generated invoices are read-only — update/delete use-cases raise `AutoGeneratedInvoiceError` → HTTP 403. The invoice list endpoint returns `funds_released_total` (sum of all `released_funds` amounts in the project). FE shows a "Funds Released" KPI card and a lock badge on auto-generated rows.

**New columns (migration `a4f5b6c7d8e9`):** `invoices.source_billing_document_id` (UUID FK → `billing_documents.id`, ON DELETE SET NULL, UNIQUE) + `invoices.is_auto_generated` (BOOLEAN, NOT NULL, default false).
