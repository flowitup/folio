...
- **Invoices** — internal expense tracking: `released_funds | labor | materials_services` invoice types, JSONB items column, per-project monthly Excel/PDF export. Auto-generated `released_funds` entries are created when a billing facture transitions to PAID (and deleted on PAID→CANCELLED); these are read-only (403 on edit/delete). Cross-BC bridge via `FundsReleasePort`. KPI row: Total / Funds Released / Labor / Materials & Services. *(Note: naming is internal only — distinct from outgoing billing below.)*
...
- **Billing module** — outgoing client-facing devis + facture documents, polymorphic on `kind`, with user-managed templates and per-document PDF export. Facture PAID status triggers auto-creation of a `released_funds` invoice in the linked project (cross-BC via `FundsReleasePort`); PAID→CANCELLED reverses it. Distinct from internal expense tracking (`invoices` table, retained under that name pending a future rename to `expenses`).
