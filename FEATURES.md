...
## 14. Billing → Funds release (cross-module auto-generation)

URL: `/{locale}/projects/{projectId}/invoices` (invoices/expenses page)

When a billing facture linked to a project transitions to **PAID**, the system auto-creates a `released_funds` invoice in the project's expenses module. The entry copies items, recipient, and issue date from the facture, and is marked `is_auto_generated=true` with `source_billing_document_id` pointing back to the facture. If the facture is later cancelled (PAID→CANCELLED, e.g. refund), the auto-generated entry is deleted.

**Key behaviors:**
- Auto-generated invoices are read-only — edit/delete returns HTTP 403 (`AutoGeneratedInvoiceError`). FE hides edit/delete buttons and shows a Lock icon badge labelled "Auto".
- `UNIQUE` constraint on `source_billing_document_id` prevents duplicate funds releases; adapter catches `IntegrityError` silently.
- Invoice list endpoint returns `funds_released_total` (sum of all `released_funds` amounts in the project).
- KPI row updated: Total / Funds Released / Labor / Materials & Services (replaces old Paid/Pending/Overdue cards).
- Cross-BC bridge: `FundsReleasePort` (billing application layer) → `FundsReleaseAdapter` (infrastructure) → `SqlAlchemyInvoiceRepository`.
- Numbering: auto-generated entries use `FR-YYYY-NNNN` sequence (distinct from user-created `INV-YYYY-NNNN`).
- i18n parity across en/fr/vi for `invoices.fundsReleased`, `invoices.auto`, `invoices.autoGenerated`.

**New columns:** `invoices.source_billing_document_id` (UUID FK UNIQUE), `invoices.is_auto_generated` (BOOLEAN)
**New file:** `app/infrastructure/adapters/funds_release_adapter.py`
**Migration:** `a4f5b6c7d8e9_add_funds_release_link_columns`
